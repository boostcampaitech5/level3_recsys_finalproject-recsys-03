...
class SongIdExtractor:
    ...
    def get_song_info(self, pl_id_list: list, sim_list: list, user_genres: list, song_k: int) -> pd.DataFrame:
        model_result = self.result_to_df(pl_id_list, sim_list)

        pl_result = self.playlist.loc[self.playlist["playlist_id"].isin(pl_id_list)][
            ["playlist_id", "playlist_songs", "num_of_songs", "playlist_view", "playlist_likecount", "tag_sit_cnt", "tag_mood_cnt", "tag_weather_cnt"]
        ]
    ...
    def spread_songs(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for id in df.playlist_id:
            ...
            for song in song_list:
                ...
                pl_like.append(l)
                pl_match.append(mr)
        pl_song = pd.DataFrame({"playlist_id": pl_id, "sim": sim, "song_id": song_id, "pl_view": pl_view, "pl_like": pl_like, "pl_match": pl_match})
